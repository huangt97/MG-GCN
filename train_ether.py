...
class Train_Model_Ether():
    def __init__(self, args):
        ...
        self.walks_train = pre_sample_for_ether(self.args.time, self.sparse_adj_train, self.id_train, self.sample_batch_size, './walk_total1/train_'+name,False)
        print('valid walk')
        self.walks_valid = pre_sample_for_ether(self.args.time, self.sparse_adj, self.id_valid, self.sample_batch_size,'./walk_total1/valid_'+name,False)
...
#                 print("loss_node",loss_node,"acc:",acc,"label_predict:",label_predict)
    ...
    def evaluation(self):
        ...
        #print('test walk')
        name = 'seed='+str(self.args.seed)+'_walk='+str(self.args.time)
        self.walks_test = pre_sample_for_ether(self.args.time, self.sparse_adj, self.id_test, self.sample_batch_size,'./walk_total1/test_'+name,False)
