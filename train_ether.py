from model import Model_Layer, MultiHeadLayer
import numpy as np
...
#         print("self.nor_graph_train_all_type",type(self.nor_graph_train_all))
        ...
        print("self.args.feature_dim = {}".format(self.args.feature_dim))
...
#         self.model = Model_Layer(self.args).cuda()
        self.model = MultiHeadLayer(self.args).cuda()
    ...
    def fit(self):
        ...
        for epoch in tqdm(range(self.args.epochs)):
            ...
            for batch in range(batch_range):
                ...
                self.epoch_loss = self.epoch_loss + self.process_batch(label, batches[batch], self.train_feature_all, self.nor_graph_train_all, dropout=0.)
#                 print("epoch_loss",self.epoch_loss)
            ...
            for batch in valid_batch:
                label = torch.index_select(self.eval_labels, 0, batch[:,0].view(-1))
                loss_node, acc, label_predict = self.process_node(label, batch, self.features, self.nor_graph, dropout=0.)
#                 print("valid loss_node",loss_node,"acc:",acc,"label_predict:",label_predict)
            ...
            if epoch % 1 == 0:
                print("epoch",epoch,"loss_train:",loss_score,"acc_train:",
                                                     self.acc_score,'||',"loss_valid:",valid_loss,"acc_valid:",valid_acc)
...
#         self.walks_test = sample_attention_weight(self.args.time, self.sparse_adj, self.id_test, self.sample_batch_size,self.args.walk_path + 'test_'+name, self.train_feature_all,self.model)
        ...
        for batch in test_batch:
            label = torch.index_select(self.test_labels, 0, batch[:,0].view(-1))
        ...
        print("loss acc:", loss_acc,"acc_acc",acc_acc, 'load epoch:', self.best_loss_epoch)
        return  loss_acc, acc_acc
    def process_node(self,label, node, feature, graph, dropout=0.):
        prediction = self.model(node, graph, feature, dropout=0.)
    ...
    def process_batch(self,label,batch, feature, graph, dropout=0.):
        self.optimizer.zero_grad()
        pre = torch.cuda.memory_allocated()
        batch_loss, acc, label_pre = self.process_node(label, batch, feature, graph, dropout=dropout)
